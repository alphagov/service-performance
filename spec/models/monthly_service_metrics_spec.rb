...
RSpec.describe MonthlyServiceMetrics, type: :model do
  describe '.between' do
    it 'returns the metrics between the start and end months, inclusive' do
      ...
      FactoryBot.create(:monthly_service_metrics, service: service, month: YearMonth.new(2017, 4))
      metrics1 = FactoryBot.create(:monthly_service_metrics, service: service, month: YearMonth.new(2017, 5))
      metrics2 = FactoryBot.create(:monthly_service_metrics, service: service, month: YearMonth.new(2017, 6))
      FactoryBot.create(:monthly_service_metrics, service: service, month: YearMonth.new(2017, 7))
  ...
  describe '#publish_date' do
    it "returns nil if there's no month" do
      metrics = FactoryBot.build(:monthly_service_metrics, month: nil)
      expect(metrics.publish_date).to be_nil
    end

    it 'returns the first of the month, 2 months from the given month' do
      metrics = FactoryBot.build(:monthly_service_metrics, month: YearMonth.new(2017, 11))
