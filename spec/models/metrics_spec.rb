...
RSpec.describe Metrics, type: :model do
  ...
  describe '#metrics' do
    it 'returns aggregated metrics for the root object' do
      ...
      expect(AggregatedCallsReceivedMetric).to receive(:new).with(root, time_period, multiplier) { calls_received_metric }
      ...
      expect(AggregatedTransactionsReceivedMetric).to receive(:new).with(root, time_period, multiplier) { transactions_received_metric }
      ...
      expect(AggregatedTransactionsWithOutcomeMetric).to receive(:new).with(root, time_period, multiplier) { transactions_with_outcome_metric }
