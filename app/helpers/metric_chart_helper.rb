module MetricChartHelper
  def calculate_transaction_difference(current_metrics, previous_metrics, total_label)
    ...
    if increased?
      "#{@current_metrics - @previous_metrics} (#{difference_increase_percentage} %) more #{total_label.gsub('Total transactions', 'transactions').downcase} than previous #{previous_metrics.count} months".html_safe
      ...
      "#{@previous_metrics - @current_metrics} (#{difference_decrease_percentage} %) less #{total_label.gsub('Total transactions', 'transactions').downcase} than previous #{previous_metrics.count} months".html_safe
    end
  end
